...
def Start_insert(x):
    ...
    log.log_Info("start insert ",str(x)+" insert")

    def exit_to_main_menu():
        log.log_Info("uzivatel",str(x)+ "exit")
        W1.destroy()
        Main_menu.start(x)
    ...
    def save_text():
        ...
        Data_handler.insert(text2,text1,"3",x,text3)
    ...
    def find_and_replace():
        search_term=simpledialog.askstring("Find and Replace","Enter the search term:")
        if search_term is not None:
            replace_term=simpledialog.askstring("Find and Replace","Enter the replacement term:")
    ...
    left_frame=tk.Frame(W1 , bg="#EEE6CE" )
    left_frame.pack(side=tk.LEFT,padx=50)

    right_frame=tk.Frame(W1 , bg="#EEE6CE")
    right_frame.pack(side=tk.RIGHT,padx=50)

    username_label=tk.Label(W1,text="Nazev:",font=("Helvetica",14),bg="#EEE6CE")
    ...
    create_tooltip(addtext_button,"nahraje text ze souboru")
...
# delete tajne informace podle id
